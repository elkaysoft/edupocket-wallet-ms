...
{
    public class GetWalletbyWalletNumberQueryHandler : IRequestHandler<GetWalletByWalletNumberQuery, GetWalletQueryResponse>
    {
        private readonly IAsyncRepository<Wallet> _walletRepository;
        private readonly IAsyncRepository<Profile> _profileRepository;

        public GetWalletbyWalletNumberQueryHandler(IAsyncRepository<Wallet> walletRepository,
        ...
        public async Task<GetWalletQueryResponse> Handle(GetWalletByWalletNumberQuery request, CancellationToken cancellationToken)
        {
            var response = new GetWalletQueryResponse();

            try
            {
                var wallet = await _walletRepository.GetSingleAsync(x => x.WalletNumber == request.walletNumber);
                if (wallet == null)
